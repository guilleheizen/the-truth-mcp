...
Entrada: el estado actual de la bóveda (CLAUDE.md + raw/seed/* + wiki/**).
...
SYSTEM_INSTRUCTION = """\
Sos el "bibliotecario" de una bóveda LLM Wiki estilo Karpathy. Tu único trabajo
es mantener la carpeta `wiki/` ordenada, consistente y bien interconectada,
contra las fuentes en `raw/seed/`.

Recibís en cada request:
1. El schema vivo de la bóveda (`CLAUDE.md`) — fuente de verdad sobre convenciones.
2. El listado completo de páginas en `wiki/` con su contenido.
3. El listado de fuentes en `raw/seed/` con su contenido.
...
- Front-matter completo en cada `create_page` o `update_page`:
  title, type (concepto|persona|paper), created (YYYY-MM-DD), updated, sources, related.
- Wikilinks `[[slug-en-kebab-case]]`. Sin tildes, sin espacios.
- Citas a `raw/` al final del párrafo factual: `(raw/seed/foo.md)`.
- Al menos 1 wikilink por página.
...
def _build_user_prompt() -> str:
    ...
    # raw/seed/
    parts.append("\n# raw/seed/ — fuentes inmutables\n")
    root = vault.vault_root()
    raw_files = vault.list_raw_seed()
    if not raw_files:
        parts.append("_(vacío)_\n")
    for rel in raw_files:
        content = (root / rel).read_text(encoding="utf-8")
        is_pending = rel in status["raw_seed_pending"]  # type: ignore[operator]
