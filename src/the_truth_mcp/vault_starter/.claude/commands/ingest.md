...
1. **Conseguí el contenido**:
   - URL → `WebFetch` (o `context7` si es doc de librería) → extraé el texto markdown.
   - Ruta a archivo → leelo.
   - Texto en bruto pasado como argumento → usalo directo.

2. **Sacá un título humano** del contenido (ej: "Software 2.0 — Karpathy").
   El slug se deriva automáticamente.
...
3. **Llamá `save_info`** del MCP `the-truth`:
   ...
   ```
   Esto:
   - Guarda en `raw/seed/<slug>.md` (inmutable).
   - Dispara al bibliotecario Gemini, que reorganiza `wiki/` para reflejar la
     nueva fuente.
...
- **Una sola tool, un solo paso**. No hay "reorganize" separado — Gemini se
  dispara solo.
- Vos no escribís en `wiki/`. Nunca. Es Gemini quien lo hace.
- Si el usuario querés revisar antes de guardar, pedile feedback sobre el título
